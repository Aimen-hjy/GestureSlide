...
最终演示模型使用本地摄像头采集数据训练。项目保留外部数据导入脚本作为实验工具，但最终推荐路线不使用 RPS 或 HaGRID 数据，因为实测后它们与本地演示场景存在分布差异，直接合入最终模型不如本地数据稳定。
...
不建议在最终模型中合入 RPS 或 HaGRID 数据。它们可以作为实验工具保留，但不写入最终演示路线。
